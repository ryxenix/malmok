...
| `failover` | 서버 3대로 구축한 뒤 **VIP 를 들고 있는 서버를 재부팅**한다 — VIP 가 남은 서버로 넘어가야 하고, 그 서버가 내려가 있는 동안 VIP 로 쓰기 요청이 들어가야 하고(etcd 3개 중 2개의 합의), 재부팅한 서버가 누가 손대지 않아도 돌아와 셋 다 Ready 가 돼야 통과. RKE2 를 강제 종료하는 방식은 쓰지 않는다: 기계가 살아 있어 VIP 가 그 서버에 남고, 그건 서버가 사라지는 장애와 다른 장애다. 아무 서버나 멈추면 아무도 쓰지 않던 노드를 잃어도 버틴다는 것밖에 증명하지 못한다 |
