...
`cluster.yaml`을 미리 작성할 필요가 없습니다.

```bash
# 작업 PC에 말목 설치 — 이미 설치했다면 생략
...
대상 노드 주소와 SSH 접근 정보를 마법사에 입력합니다. 마법사가 구성 파일을
생성하고 CLI와 같은 엔진으로 구축을 진행합니다. 말목 설치 후 노드에 접속하지
않고 화면만 체험하려면 `malmok apply --demo --lang ko`를 실행하십시오.

수동 다운로드와 폐쇄망 환경은 [설치 옵션](installation.md)을 확인하십시오.

## 다른 방법: 구성 파일로 반복·자동화하기

파일로 구성을 관리하려면 아래 경로를 선택하십시오. 마법사의 선행 작업이 아니며,
마법사에서 구축을 완료했다면 다시 설치할 필요는 없습니다.
