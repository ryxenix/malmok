...
!!! warning "알파 단계"
    ...
    단일 노드와 2노드 구성은 실제 장비에서 반복 검증했습니다. 3서버 HA,
    프록시와 외부 레지스트리 미러는 미검증입니다. 에어갭은 반입 아티팩트로
    RKE2·Cilium·Gateway API를 세우는 경로까지 검증됐고, 플랫폼 차트를 파일로
    반입하는 경로는 아직 끝까지 검증되지 않았습니다. 자세한 범위는
    [에어갭 가이드](guides/air-gap.md)에 있습니다. 문서 스키마는
