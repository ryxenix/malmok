...
- 플랫폼이 받아오는 이미지. 아키텍처별 파일 하나
  `malmok-images_<tag>_linux_<arch>.tar.zst` 가 전부 담고 있으며, 바이너리가
  출력하는 그 목록으로 만들어집니다. 번들은 그 목록이 바뀔 때만 새로
  만듭니다. 이미지가 이전 릴리스와 같으면 릴리스 노트가 그 사실과 함께 이전
  릴리스의 번들 링크·체크섬을 알려주므로, 파일 이름의 `<tag>` 가 설치하는
  릴리스보다 오래된 것일 수 있습니다. 각 노드
