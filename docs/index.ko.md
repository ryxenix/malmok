...
!!! warning "알파 단계"
    ...
    홈랩이나 평가 환경에서 시작하십시오. 업무 도입 전에는
    [검증 매트릭스](40-verification-matrix.md)와 [에어갭 요구사항](guides/air-gap.md)을
    확인하십시오. `v1alpha1` 스키마는 마이너 릴리스 사이에 바뀔 수 있습니다.
...
서버가 준비되어 있어도 현장별 네트워크 점검, 자재 준비와 결과 인계는 남습니다.
개발자는 초기 말목으로 실제 IDC 서버에 클러스터를 구축했으며, 이 경험을 바탕으로
점검·재개·인계 흐름을 발전시키고 있습니다. 모든 업무 환경의 안정성을 보증하는 것은 아닙니다.

[온프레미스 구축 체크리스트 →](guides/on-premises.md) · [에어갭 지원 조건 →](guides/air-gap.md) · [업그레이드와 복구 →](guides/upgrade-and-recovery.md)
