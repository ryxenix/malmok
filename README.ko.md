...
**개인·업무 환경 모두 도입 전 [검증 범위](#검증-범위)를 확인하십시오.
현재 알파 단계이며, 위 사용 사례가 프로덕션 준비 완료를 뜻하지는 않습니다.**
...
| 에어갭 설치 (외부 통신 차단) | **하드웨어 검증** | 일반 매트릭스 밖의 전용 2노드 랩 실행. egress를 REJECT가 아닌 DROP으로 차단(현장 방화벽과 같은 방식), RKE2·Cilium·Gateway API 모두 반입 아티팩트로, 나가려 한 시도는 전부 로깅해 규명 |
| 프록시 환경 | **미검증** | 스키마만 존재 |
