...
<p align="center"><sub>베어메탈 · VM · 온프레미스 · DMZ · 에어갭(<a href="#검증-범위">일부 검증</a>)</sub></p>
...
> 장비에서 반복 검증했지만 **3서버 HA·프록시·외부 레지스트리는 미검증**이고,
> 에어갭은 반입 아티팩트로 RKE2·Cilium·Gateway API를 세우는 경로까지만
> 검증됐으며 플랫폼 차트를 파일로 반입하는 경로는 아직입니다. 문서 스키마는
> `v1alpha1`이며 마이너 릴리스 사이에 바뀔 수 있습니다.
...
| 에어갭 설치 (외부 통신 차단) | **하드웨어 검증, 일부** | 2노드 랩 실행, egress를 현장 방화벽처럼 DROP. 반입 아티팩트로 RKE2·Cilium·Gateway API 구축, 나가려 한 시도 전부 규명. **플랫폼 차트를 파일로 반입하는 경로(`registry.chartDir`)는 아직 끝까지 검증되지 않음** — 전달 경로를 0.92.0에서 고쳤고 랩 케이스는 그 뒤 통과한 적 없음 |
| 프록시 환경 | **미검증** | 스키마만 존재 |
