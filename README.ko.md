...
> 장비에서 업그레이드·재개·재적용까지 반복 검증했지만 **3서버 HA·프록시·외부
> 레지스트리는 미검증**입니다. 에어갭은 검증됐습니다 — egress를 끊은 상태에서
...
아래 각 행은 검증 매트릭스가 명시된 릴리스에서, 노드 2대로, 명시된 네트워크
조건으로 실제 수행한 결과입니다. 0.95.0 기준 10개 케이스가 전부 통과합니다 —
에어갭이 마지막이었고, 거기 도달하기까지 결함 다섯 개를 더 드러냈습니다.
