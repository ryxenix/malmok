...
<p align="center"><img src="docs/img/malmok-mascot-wordmark.png" alt="말목 — 미소 짓는 파란 말뚝 마스코트" width="620"></p>
...
Linux 서버의 SSH 접근 권한과 원하는 구성을 적은 `cluster.yaml`을 주면,
말목이 호스트를 준비하고 RKE2와 선택한 데이터플레인·게이트웨이·플랫폼 구성요소를
설치합니다. 중단된 구축은 이어서 진행하고, 실행 기록은 문제 해결과 인수인계에
활용할 수 있도록 남깁니다. 구성 파일이 없다면 마법사로 함께 만들 수 있습니다.

> **상태: 알파.** 홈랩이나 평가 환경에서 시작하십시오. 업무 도입 전에는
> [검증 범위](#검증-범위)를 확인하십시오. `v1alpha1` 스키마는 마이너 릴리스
> 사이에 바뀔 수 있습니다.
...
RKE2 부트스트랩과 선택한 Cilium 또는 Canal 데이터플레인, Gateway API,
cert-manager, ArgoCD, VictoriaMetrics 관측 스택을 설정에 따라 처리합니다.
...
# certs가 출력한 실행 ID로 감사 보고서와 handoff.json 생성
...
단일·2노드 구성은 실제 장비에서 구축·업그레이드·재개·재적용을 검증했습니다.
서버 3대 구성은 구축, 서버 하나를 잃는 장애, 업그레이드까지 검증했습니다. 에어갭은 egress를
끊고 RKE2 자재·차트·이미지를 반입 파일로 설치했습니다. 프록시·외부 레지스트리
미러는 미검증이며, 정확한 조건과 릴리스별 근거는 아래 표를 따릅니다.

버전은 호환성 약속이지 완성도 점수가 아닙니다. `0.x`에서는 스키마가 바뀔 수
있습니다. 1.0의 목표는 `v1` 스키마와 남은 미검증 항목의 검증입니다.
진행 내용은 [변경 이력](CHANGELOG.md)을 확인하십시오.

인프라 도구가 과장하면 남의 클러스터가 깨집니다. 그래서 검증된 것과
안 된 것을 나눠 적습니다.

아래 각 행은 검증 매트릭스가 명시된 릴리스에서, 명시된 노드 수와 네트워크
조건으로 실제 수행한 결과입니다. 12개 케이스 중 10개가 0.95.0 기준으로
통과합니다 — 에어갭이 마지막이었고, 거기 도달하기까지 결함 다섯 개를 더
드러냈습니다. 11번째인 서버 3대 장애 케이스는 그 뒤에 추가됐고 0.96.3에서,
12번째인 서버 3대 업그레이드는 0.96.4에서 통과합니다.
...
| 에어갭 설치 (외부 통신 차단) | **실측 검증** | 매트릭스 `airgap-pair`, 0.96.4: egress를 현장 방화벽처럼 DROP. 반입 아티팩트로 RKE2·Cilium, 파일로 반입한 플랫폼 차트(`registry.chartDir`), 반입 번들에서 전 이미지, local-path 볼륨 바인딩, 메트릭 DB 구동까지 — 밖에서 받아온 것 없음. 이 릴리스에서 12개 중 9개는 한 번의 연속 스위트로 실행했고, 이 케이스와 `canal-pair`·`upgrade-three`는 같은 빌드에서 단독 실행. 12개를 한 번에 쓸어담은 적은 아직 없음 |
...
| GPU 구성 | device plugin·RuntimeClass·GPU 컨테이너 런타임을 설치하지 않습니다. `topology[].gpu`를 설정한 문서는 파드가 하드웨어를 보지 못하는 클러스터를 만드는 대신 거절합니다. 클러스터를 세운 뒤 벤더 오퍼레이터를 직접 설치하십시오 |
| 애플리케이션 배포 | ArgoCD를 설치하고 저장소를 가리키게 할 뿐, 무엇을 동기화할지는 사용자 몫 |
| 노드 방화벽 변경 | preflight가 활성 방화벽과 필요한 포트를 보고합니다. 정책은 그것을 소유한 쪽의 몫 |
| 운영 업무 전체 대행 | 업그레이드와 현재 main의 인증서 만료 점검은 지원합니다. 상시 감시·알림 대응·유지보수 일정 관리는 운영자 책임입니다 |
