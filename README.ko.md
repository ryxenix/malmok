...
드러냈습니다. 11번째인 서버 3대 장애 케이스는 그 뒤에 추가됐고 main에서
통과합니다. 필요했던 수정은 아직 릴리스되지 않았습니다.
...
| 3서버 HA (etcd 쿼럼) | **실측 검증** | 매트릭스 `ha-failover`, 0.96.1 이후 main(미릴리스): VIP 아래 서버 3대, VIP를 쥔 서버를 재부팅 — VIP 이전, etcd 3개 중 2개로 쓰기 성공, 재부팅한 서버가 스스로 복귀. 서버 3대 업그레이드는 미검증. `rke2-killall.sh`로 서버를 멈추면 VIP가 그 서버에 남음 — [복구 가이드](docs/guides/upgrade-and-recovery.ko.md) |
